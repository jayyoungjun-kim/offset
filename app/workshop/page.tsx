...
const faqs: {q:string; a:React.ReactNode}[] = [
  ...
  { q: "왜 그룹당 최대 4명으로 운영하나요?", a: "모든 포트폴리오를 충분한 시간 동안 깊이 리뷰하기 위해서입니다. 인원이 많아질수록 피드백은 얕아지고, 수정 과정을 함께 살펴보기도 어려워집니다. 포트폴리오 상태와 고민이 잘 맞는 참가자끼리 2~4명으로 그룹을 구성하며, 인원을 채우기 위해 서로 맞지 않는 참가자를 억지로 묶지 않습니다. 최대 4개 그룹까지 운영합니다." },
  ...
  { q: "신청하면 바로 참여할 수 있나요?", a: "신청서는 접수된 순서대로 검토합니다. 제출한 포트폴리오와 신청 내용을 바탕으로 이 과정에서 실질적인 도움을 받을 수 있는 분들을 선정하고, 포트폴리오 상태와 현재 고민이 비슷한 참가자끼리 그룹을 구성합니다. 선정 안내 후 참가비 결제가 완료되어야 참여가 최종 확정되며, 그룹별 결제 인원이 확정되면 모집을 종료합니다." },
  ...
  return <main className="workshop-page">
    ...
    <div className="page-body">
      ...
      <Section title="어떻게 진행되나요">
        ...
        <p>포트폴리오 상태와 현재 고민이 비슷한 2~4명의 주니어 프로덕트 디자이너로 그룹을 구성하며, 최대 4개 그룹까지 운영합니다. 모든 리뷰는 멘토가 진행하며, 참가자는 다른 포트폴리오가 어떻게 개선되는지도 함께 보게 됩니다. 다른 사람의 피드백을 듣는 과정은 자신의 포트폴리오를 객관적으로 바라보는 가장 좋은 기준이 됩니다. 포트폴리오 상태와 현재 고민이 비슷한 참가자끼리 그룹을 구성하는 이유도 같습니다. 같은 고민을 가진 사람들과 함께할 때 가장 많은 것을 얻을 수 있습니다.</p>
      </Section>
      <Section title="멘토 소개">
      ...
      <Section title="워크샵 정보">
        <p>4주 동안 오프라인으로 진행되며, 그룹당 최대 4명, 최대 4개 그룹으로 운영합니다. 신청 시 현재 포트폴리오를 제출해야 하며, 매주 받은 피드백을 다음 모임 전까지 반영하는 것을 원칙으로 합니다.</p>
