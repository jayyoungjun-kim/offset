...
const steps = ["신청서를 작성해주세요.", "제출해주신 포트폴리오와 신청 내용을 검토합니다.", "참가 가능 여부를 이메일과 문자로 개별 안내드립니다.", "참가비 결제 후 참가가 최종 확정됩니다."];
...
const faqs: {q:string; a:React.ReactNode}[] = [
  ...
  { q: "왜 최대 4명만 모집하나요?", a: "모든 포트폴리오를 충분한 시간 동안 깊이 리뷰하기 위해서입니다. 인원이 많아질수록 피드백은 얕아지고, 수정 과정을 함께 살펴보기도 어려워집니다. 4명은 각자의 포트폴리오를 충분히 다루면서도 다른 사람의 리뷰를 함께 배울 수 있는 가장 적절한 규모라고 생각합니다." },
  ...
  { q: "신청하면 바로 참여할 수 있나요?", a: "신청서를 제출하면 내용을 검토한 뒤 개별적으로 안내드립니다. 참가가 확정되면 결제를 진행하며, 그룹이 구성된 후 워크숍이 시작됩니다." },
  ...
  return <main className="workshop-page">
    ...
    <div className="page-body">
      ...
      <Section title="어떻게 진행되나요">
        ...
        <p>그룹당 4명의 주니어 프로덕트 디자이너로 구성됩니다. 모든 리뷰는 멘토가 진행하며, 참가자는 다른 포트폴리오가 어떻게 개선되는지도 함께 보게 됩니다. 다른 사람의 피드백을 듣는 과정은 자신의 포트폴리오를 객관적으로 바라보는 가장 좋은 기준이 됩니다. 비슷한 수준의 디자이너만 모집하는 이유도 같습니다. 같은 고민을 가진 사람들과 함께할 때 가장 많은 것을 얻을 수 있습니다.</p>
      </Section>
      <Section title="멘토 소개">
      ...
      <Section title="워크샵 정보">
        <p>4주 동안 오프라인으로 진행되며, 그룹당 4명이 참여합니다. 신청 시 현재 포트폴리오를 제출해야 하며, 매주 받은 피드백을 다음 모임 전까지 반영하는 것을 원칙으로 합니다.</p>
        <dl className="lined-list">{workshopInfo.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
      </Section>
      <Section title="신청 절차">
        <p>아래에 절차대로 신청을 진행해주세요. 선착순으로 검토하고 개별 안내드립니다.</p>
