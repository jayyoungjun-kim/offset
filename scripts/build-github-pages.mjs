...
const analytics = gaMeasurementId
  ...
  document.addEventListener("click", event => {
    ...
    const item = link.classList.contains("nav-logo") ? "logo" : destination === "/about" ? "about" : destination === "/workshop" ? "workshop" : "";
...
const channelTalk = String.raw`(() => {
  ...
  channel("onShowMessenger", () => {
    window.gtag?.("event", "channel_talk_open", {page_path:location.pathname});
  });
...
const workshopCta = String.raw`(() => {
  ...
  cta.addEventListener("click", () => window.gtag?.("event", "apply_cta_click", {cta_location:"workshop",link_url:"/apply"}));
})();`;
...
const applicationForm = String.raw`(() => {
  ...
  const track = (event, properties = {}) => window.gtag?.("event", event, properties);
  const trackStart = () => {
    if (hasStarted) return;
    hasStarted = true;
    track("application_start", {form_name:"portfolio_workshop"});
  };
  ...
  fileInput?.addEventListener("change", () => {
    ...
    track("portfolio_upload", {file_type:file.type || "unknown"});
  });
  ...
  form.addEventListener("submit", async event => {
    ...
    if (errors.length) {
      track("application_validation_error", {error_count:errors.length,first_error:errors[0][0]});
      fieldFor(errors[0][0])?.scrollIntoView({behavior:"smooth",block:"center"}); return;
    }

    clearSubmitError(); submitButton.disabled = true;
    track("application_submit", {form_name:"portfolio_workshop",has_file:!!file,has_portfolio_link:!!values.portfolioLink});
    ...
    try {
      ...
      track("application_complete", {form_name:"portfolio_workshop"});
      location.assign("/apply/complete/");
    } catch (error) {
      track("application_submit_error", {form_name:"portfolio_workshop"});
